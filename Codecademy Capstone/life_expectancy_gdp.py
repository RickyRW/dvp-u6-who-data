...
plt.savefig("bar_plot_LEABY.png")


# What do you notice about the two bar charts? Do they look similar?

# The relation of GDP to LEABY doesn't appear to have a distinct pattern.
...
#  I see that Zimbabwe experienced some growth in Life Expectancy over the years, but had very little growth in GDP. This would explain Zimbabwe's overall lower average of LEABY in relation to GDP in comparison to the other countries.
...
ax = sns.barplot(x="Country",y="GDP",hue="Year",data=df, color="blue")
...
f, ax = plt.subplots(figsize=(10, 15)) 
ax = sns.barplot(x="Country",y="LEABY",hue="Year",data=df,color="red")
plt.xticks(rotation=90)
...
# 2012 to 2015 shows the larger changes
# Zimbabwe had the least change in GDP over time
# The US and China show the largest changes in GDP in comparison to the other countries, but all countries show similar increases in LEABY
# As GDP increases, LEABY also tends to increase. However, Zimbabwe's LEABY increased despite a lack of GDP increase, contradicting the relationship of GDP to LEABY
# I would think that the evolution of technology and foreign policies would play a role in Zimbabwe's data
...
plt.savefig("facet_scatter_gdp.png")
...
# China moves the most along the x axis
# Zimbabwe moves the most along the y axis
# China's movement is surprising due to their very rapid increase in GDP. Their rate of increase is very high in comparison to other countries
...
plt.savefig("line_plots_LEABY.png")
...
# Zimbabwe changes the most
# The largest change in data appears to be from 2008 to 2015
# Mexico has had the least change in LEABY
# Increased technology in healthcare and more developed foreign policies may explain Zimbabwe's data
...
plt.savefig("line_plots_gdp.png")
...
# From https://www.scmp.com/comment/insight-opinion/article/3004415/chinas-decade-extraordinary-growth-2008-lost-its-critics;
# 
...
# I think each country's economic goals contributed to both factors: wanting to improve economic conditions by employing newer strategies and tactics led to greater growth(china, us), while a lack of adaptation or planning hindered growth (chile,mexico)
# More variables to compare would have made the project more reliable. Only using GDP to compare to LEABY presents a very limited perspective.
# I think the scatter plots best showed the relationships between GDP and LEABY on one graph.
